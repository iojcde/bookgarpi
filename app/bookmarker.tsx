    ...
    <>
      <form
        action={async (formData) => {
          try {
            new URL(formData.get("url") as string);
          } catch {
            return;
          }

          const newgarpi = await createGarpi(formData.get("url") as string);
          if (!newgarpi) {
            alert("Invalid URL");
          }
          setGarpis([
        ...
        className="flex mt-8 rounded-xl  shadow-inner border border-gray-5 ring-offset-2 py-2 ring-gray-6 ring-offset-gray-1 px-3 items-center gap-3 bg-gray-2"
      >
